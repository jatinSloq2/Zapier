// triggers/newIncomingMessage.js (ES6)
...
const triggerNewIncomingMessage = async (z, bundle) => {
  ...
  if (response.data.users && response.data.users.data) {
    response.data.users.data.forEach((user) => {
      if (user.last_chat && user.last_chat.type === 'recieve') {
        messages.push({
...
const triggerNewIncomingMessageHook = async (z, bundle) => {
  // For webhook-based triggers (if you implement webhooks)
...
export default {
  ...
  operation: {
    type: 'polling', // Change to 'hook' if implementing webhooks
